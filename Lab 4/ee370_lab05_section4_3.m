...
nfreq = 1515; %2*N + 1; % includes negative frequencies and zero
fstrt = -fmax;
fstop = fmax;
...
for n = -N:N
    ...
    sum_value_open = sum_value_open + S11_open_windowed(n+N+1) * exp(1j * n * (2 * pi * omega_step) * time);

    % S11 Short
    S11_short_windowed = S11_short .* W;
    sum_value_short = sum_value_short + S11_short_windowed(n+N+1) * exp(1j * n * (2 * pi * omega_step) * time);
end

f_t_open = (1 / W0) * sum_value_open;
f_t_short = (1 / W0) * sum_value_short;
...
% For S11 OPen
S11_mag = mag2db(sqrt((real(S11_open)).^2 + (imag(S11_open)).^2));%mag2db(abs(S11_open));
S11_phase = rad2deg(atan2(imag(real(S11_open)), real(real(S11_open))));%rad2deg(angle(S11_open));
...
S11_mag = mag2db(sqrt((real(S11_short)).^2 + (imag(S11_short)).^2));%mag2db(abs(S11_open));
S11_phase = rad2deg(atan2(imag(real(S11_short)), real(real(S11_short))));%rad2deg(angle(S11_open));
